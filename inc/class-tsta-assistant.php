<?php
/**
 * Tista Assistant class.
 *
 * @package Tista_Assistant
 */
...
if ( ! class_exists( 'Tista_Assistant' ) ) :

	/**
	 * It's the main class that does all the things.
	 *
	 * @class Tista_Assistant
	 * @version 4.2.1
	 * @since 1.0.0
	 */
	final class Tista_Assistant {
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'tista-assistant' ), '1.0.0' );
		}
		...
		private function init_globals() {
			...
			$this->version     = TISTA_ASSISTAT_VERSION;
			$this->slug        = 'tista-assistant';
			$this->option_name = self::sanitize_key( $this->slug );
			$this->plugin_url  = TISTA_ASSISTAT_URI;
			$this->plugin_path = TISTA_ASSISTAT_PATH;
			$this->page_url    = TISTA_ASSISTAT_NETWORK_ACTIVATED ? network_admin_url( 'admin.php?page=' . $this->slug ) : admin_url( 'admin.php?page=' . $this->slug );
		...
		private function init_includes() {
			...
			require $this->plugin_path . '/inc/widget/class-widget-search.php';
			require $this->plugin_path . '/inc/widget/class-widget-sidebar-category.php';
			require $this->plugin_path . '/inc/widget/class-widget-sidebar-post.php';
			require $this->plugin_path . '/inc/widget/class-widget-tags.php';			
			require $this->plugin_path . '/inc/widget/class-widget-address.php';
			require $this->plugin_path . '/inc/widget/class-widget-usefulllink.php';
			require $this->plugin_path . '/inc/class-tista-contact.php';
			require $this->plugin_path . '/inc/class-tista-shortcode.php';
		}
		...
		private function init_actions() {
			// Activate plugin.
			register_activation_hook( TISTA_ASSISTAT_CORE_FILE, array( $this, 'activate' ) );

			// Deactivate plugin.
			register_deactivation_hook( TISTA_ASSISTAT_CORE_FILE, array( $this, 'deactivate' ) );
			...
			add_action( 'wp_enqueue_scripts', array($this, 'tista_assistant_enqueue_scripts') );
		}
		...
		public function load_textdomain() {
			load_plugin_textdomain( 'tista', false, TISTA_ASSISTAT_PATH . 'languages/' );
		}
		...
		public function set_options( $options ) {
			TISTA_ASSISTAT_NETWORK_ACTIVATED ? update_site_option( $this->option_name, $options ) : update_option( $this->option_name, $options );
		}
		...
		public function get_options() {
			return TISTA_ASSISTAT_NETWORK_ACTIVATED ? get_site_option( $this->option_name, array() ) : get_option( $this->option_name, array() );
		}
		...
		public function tista_widget() {
			...
			register_widget( 'Tista_Address' );			
			register_widget( 'Tista_Usefull_Link' );			
		}
		 ...
		 * @access  public
		 */
		public function tista_assistant_enqueue_scripts() {
			wp_enqueue_style( 'tista-assistant-admin', TISTA_ASSISTAT_URI.'/assets/css/smart-forms.css', '', '','screen' );
			wp_enqueue_script( 'jquery' );
			wp_enqueue_script( 'tista-assistant-admin', TISTA_ASSISTAT_URI.'/assets/js/smart-form.js', '', '',true);
		}
	}
